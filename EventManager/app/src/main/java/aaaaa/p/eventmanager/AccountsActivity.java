...
public class AccountsActivity extends AppCompatActivity {
    ...
    private boolean validaciones1() {
        if (validacion(editTextUser) && validacion(editTextName) && validacion(editTextLast1) && validacion(editTextLast2)
                && validacion(editTextDate) && validacion(editTextEmail) && validacion(editTextPass))
            if (validEmail())
                if (editTextPass.getText().toString().equals(Utilidades.leerPreferenciaPassword(preferencias))) {
                    Toast.makeText(this, "Guardando cambios...", Toast.LENGTH_LONG).show();
                    return true;
                } else {
